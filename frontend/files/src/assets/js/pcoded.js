...
function layout_change(layout) {
    ...
    if (layout == 'dark') {
        ...
        dark_flag = false;
        if (document.querySelector('.pc-sidebar .m-header .logo-lg')) {
            document.querySelector('.pc-sidebar .m-header .logo-lg').setAttribute('src', '../../assets/images/logo_tma.png');
        }
        if (document.querySelector('.navbar-brand .logo-lg')) {
            document.querySelector('.navbar-brand .logo-lg').setAttribute('src', '../../assets/images/logo_tma.png');
        }
        if (document.querySelector('.auth-main.v1 .auth-sidefooter')) {
            document.querySelector('.auth-main.v1 .auth-sidefooter img').setAttribute('src', '../../assets/images/logo_tma.png');
        }
        if (document.querySelector('.footer-top .footer-logo')) {
            document.querySelector('.footer-top .footer-logo').setAttribute('src', '../../assets/images/logo_tma.png');
        }
        var control = document.querySelector('.theme-layout > a.active');
        if (control) {
            document.querySelector('.theme-layout > a.active').classList.remove('active');
...
// file: ../assets/js/pcoded.js

function initializeSidebarToggle() {
    var sidebar_hide = document.querySelector('#sidebar-hide');
    if (sidebar_hide) {
        sidebar_hide.addEventListener('click', function () {
            // Dùng querySelector một lần nữa ở đây để chắc chắn
        ...
        // Dòng này giúp bạn biết nếu có lỗi
        console.warn('Sidebar toggle button #sidebar-hide not found after layout inclusion.');
    }
}

function initializeMobileMenu() {
    const mobileCollapseButton = document.querySelector('#mobile-collapse');

    if (mobileCollapseButton) {
        mobileCollapseButton.addEventListener('click', function (event) {
            // Ngăn hành vi mặc định của thẻ <a> (quan trọng!)
            event.preventDefault();

            const sidebar = document.querySelector('.pc-sidebar');
            const topbar = document.querySelector('.topbar');
            const isHorizontalLayout = document.querySelector('.pc-horizontal');

            // Xác định phần tử mục tiêu dựa trên layout (ngang hay dọc)
            const targetElement = isHorizontalLayout ? topbar : sidebar;

            if (targetElement) {
                // Nếu menu đang mở, gọi hàm rm_menu() để đóng nó lại
                if (targetElement.classList.contains('mob-sidebar-active')) {
                    rm_menu();
                } else {
                    // Nếu menu đang đóng, hãy mở nó ra
                    targetElement.classList.add('mob-sidebar-active');
                    targetElement.insertAdjacentHTML('beforeend', '<div class="pc-menu-overlay"></div>');

                    // Gán sự kiện click cho lớp overlay vừa được tạo để đóng menu
                    const newOverlay = document.querySelector('.pc-menu-overlay');
                    if (newOverlay) {
        ...
        console.warn('Mobile collapse button #mobile-collapse not found after layout inclusion.');
    }
}
